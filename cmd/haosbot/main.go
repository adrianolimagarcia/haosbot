// Command haosbot is a Go reimplementation of the haosbot agent runtime.
//
// It is designed to read and write the same ~/.haosbot/ directory as the
...
// version is the haosbot version. It is deliberately distinct from the
...
func main() {
	...
	default:
		fmt.Fprintf(os.Stderr, "haosbot: unknown command %q\n\n", os.Args[1])
		usage(os.Stderr)
		os.Exit(2)
	}
}
...
func printVersion() {
	fmt.Printf("haosbot %s\n", version)
	fmt.Printf("go         %s %s/%s\n", runtime.Version(), runtime.GOOS, runtime.GOARCH)
	fmt.Printf("compat     %s\n", compatibilityTarget)
}

// printPaths shows the resolved runtime layout, mirroring
// nanobot/config/paths.py. It is the fastest way to confirm that haosbot
// and the Python haosbot would read the same files.
func printPaths() error {
	home, err := os.UserHomeDir()
	if err != nil {
		return fmt.Errorf("resolve home directory: %w", err)
	}
	dataDir := filepath.Join(home, ".haosbot")
...
func usage(w *os.File) {
	fmt.Fprint(w, `haosbot — Go runtime for the haosbot agent framework

Usage:
  haosbot run "<msg>"    Send one message and print the reply
  haosbot chat           Interactive REPL
  haosbot gateway        Run the agent loop until interrupted
  haosbot version        Print version and compatibility target
  haosbot paths          Show the resolved ~/.haosbot/ layout
  haosbot selftest       Build the core runtime in-process and report sizes
  haosbot help           Show this message
...
either. Configure the key in ~/.haosbot/config.json.
...
// silently reports zero. Use: haosbot selftest --hold 1s
func selftest(args []string) error {
	...
	builder := prompt.New(filepath.Join(homeDir(), ".haosbot", "workspace"))
...
func fatal(err error) {
	fmt.Fprintf(os.Stderr, "haosbot: %v\n", err)
	os.Exit(1)
}
