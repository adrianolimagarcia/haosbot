...
func (s *Server) registerAgentTurnStream(mux *http.ServeMux) {
	mux.HandleFunc("/api/agent/turn/stream", func(w http.ResponseWriter, r *http.Request) {
		...
		resultCh := make(chan struct { out *core.OutboundMessage; err error }, 1)
		hook := &agentTurnStreamHook{ctx:ctx, events:events}
		go func() {
			defer close(events)
			out, err := loop.ProcessMessageWithHook(ctx, core.InboundMessage{Channel:"webui", SenderID:sessionID, ChatID:sessionID, Content:message, Media:media, Timestamp:time.Now(), Metadata:map[string]any{"source":"webui", "session_id":sessionID}}, hook)
			resultCh <- struct { out *core.OutboundMessage; err error }{out:out, err:err}
		}()
		encoder := json.NewEncoder(w)
		for event := range events { if err := encoder.Encode(event); err != nil { return }; flusher.Flush() }
		result := <-resultCh
		if result.err != nil { event := agentTurnStreamEvent{Type:"error", Error:result.err.Error()}; if err := encoder.Encode(event); err == nil { flusher.Flush() }; return }
		content := ""; if result.out != nil { content = result.out.Content }
		_ = encoder.Encode(agentTurnStreamEvent{Type:"done", SessionID:sessionID, Content:content, StopReason:"completed"}); flusher.Flush()
	})
}
