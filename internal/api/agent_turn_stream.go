...
type agentTurnStreamHook struct {
	agent.NopHook
	ctx    context.Context
	events chan<- agentTurnStreamEvent
}

func (h *agentTurnStreamHook) emit(event agentTurnStreamEvent) {
	select {
	case h.events <- event:
	case <-h.ctx.Done():
	}
...
func (h *agentTurnStreamHook) OnReasoningDelta(ctx context.Context, delta string) {
	h.emit(agentTurnStreamEvent{Type: "reasoning_delta", Delta: delta})
}

func (h *agentTurnStreamHook) OnReasoningEnd(ctx context.Context) {
	h.emit(agentTurnStreamEvent{Type: "reasoning_end"})
}

func (h *agentTurnStreamHook) OnToolStart(ctx context.Context, call core.ToolCall) {
	h.emit(agentTurnStreamEvent{Type: "tool_start", ToolCallID: call.ID, ToolName: call.Name})
}
...
func (s *Server) registerAgentTurnStream(mux *http.ServeMux) {
	mux.HandleFunc("/api/agent/turn/stream", func(w http.ResponseWriter, r *http.Request) {
		...
		defer cancel()
		events := make(chan agentTurnStreamEvent, 64)
		resultCh := make(chan struct {
			out *core.OutboundMessage
			err error
		}, 1)
		hook := &agentTurnStreamHook{ctx: ctx, events: events}
		go func() {
			defer close(events)
		...
		for event := range events {
			if err := encoder.Encode(event); err != nil { return }
			flusher.Flush()
		}
		result := <-resultCh
		if result.err != nil {
			event := agentTurnStreamEvent{Type: "error", Error: result.err.Error()}
			if err := encoder.Encode(event); err == nil { flusher.Flush() }
			return
		}
		content := ""
		if result.out != nil { content = result.out.Content }
		_ = encoder.Encode(agentTurnStreamEvent{
			Type: "done", SessionID: sessionID, Content: content, StopReason: "completed",
		})
		flusher.Flush()
	})
}
