...
func testAttachmentServer(t *testing.T) *Server { t.Helper(); s:=NewServer(&config.Config{},nil,nil); s.SetDataDir(t.TempDir()); return s }

func TestValidateWebUIMediaScopesFilesToSession(t *testing.T) {
	s:=testAttachmentServer(t); session:="session_1234567890"; dir:=filepath.Join(s.webUIMediaRoot(),session)
	if err:=os.MkdirAll(dir,0o700);err!=nil{t.Fatal(err)}
	inside:=filepath.Join(dir,"0123456789abcdef01234567.txt"); if err:=os.WriteFile(inside,[]byte("hello"),0o600);err!=nil{t.Fatal(err)}
	got,err:=s.validateWebUIMedia(session,[]string{inside,inside}); if err!=nil{t.Fatalf("validate inside: %v",err)}
	if len(got)!=1||got[0]!=inside{t.Fatalf("dedupe got %#v",got)}
	outside:=filepath.Join(filepath.Dir(s.webUIMediaRoot()),"outside.txt"); if err:=os.WriteFile(outside,[]byte("no"),0o600);err!=nil{t.Fatal(err)}
...
func TestAttachmentListResolveAndCleanup(t *testing.T) {
	s:=testAttachmentServer(t); session:="session_1234567890"; dir:=filepath.Join(s.webUIMediaRoot(),session)
	if err:=os.MkdirAll(dir,0o700);err!=nil{t.Fatal(err)}
	id:="0123456789abcdef01234567"; path:=filepath.Join(dir,id+".txt"); if err:=os.WriteFile(path,[]byte("preview me"),0o600);err!=nil{t.Fatal(err)}
	items,err:=s.listWebUIAttachments(session); if err!=nil{t.Fatal(err)}
	if len(items)!=1||items[0].ID!=id||items[0].Binary||items[0].Preview!="preview me"{t.Fatalf("unexpected item %#v",items)}
	resolved,err:=s.resolveWebUIAttachment(session,id); if err!=nil||resolved!=path{t.Fatalf("resolve %q %v",resolved,err)}
