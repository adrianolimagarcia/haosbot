...
func (s *Server) handleWebUITemporary(w http.ResponseWriter,r *http.Request){
	...
	case http.MethodPost:
		var raw [16]byte
		if _,err:=rand.Read(raw[:]);err!=nil{http.Error(w,err.Error(),http.StatusInternalServerError);return}
		id:="tmp_"+hex.EncodeToString(raw[:]);key:="webui:"+id
		if _,err:=session.DefaultTransientStore().Open(key);err!=nil{http.Error(w,err.Error(),http.StatusInternalServerError);return}
		writeWebUIJSON(w,map[string]any{"session_id":id,"key":key,"temporary":true})
	case http.MethodDelete:
		id:=strings.TrimSpace(r.URL.Query().Get("session_id"))
		if !strings.HasPrefix(id,"tmp_")||!webSessionIDPattern.MatchString(id){http.Error(w,"invalid temporary session",http.StatusBadRequest);return}
