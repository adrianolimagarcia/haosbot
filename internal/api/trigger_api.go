...
func (s *Server) handleWebUITriggers(w http.ResponseWriter, r *http.Request) {
	service := s.triggers.Load()
	if service == nil {
		http.Error(w, "trigger service unavailable", http.StatusServiceUnavailable)
		return
	}
	switch r.Method {
	case http.MethodGet:
		sessionKey := strings.TrimSpace(r.URL.Query().Get("session_key"))
		items := service.List(true)
		if sessionKey != "" {
			filtered := make([]triggersruntime.Trigger, 0)
			for _, tr := range items {
				if tr.SessionKey == sessionKey {
	...
	case http.MethodPost:
		...
		tr, err := service.Create(req.Name, req.Channel, req.ChatID, req.SessionKey, map[string]any{"source": "webui", "session_id": strings.TrimPrefix(req.SessionKey, "webui:")})
		if err != nil {
...
func (s *Server) handleWebUITrigger(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPatch:
		...
		tr, err := service.Update(id, req.Name, req.Enabled)
...
func (s *Server) handleWebUITriggerFire(w http.ResponseWriter, r *http.Request) {
	...
	d, err := service.Enqueue(id, req.Content)
	if errors.Is(err, os.ErrNotExist) {
		http.NotFound(w, r)
		return
	}
	...
	writeWebUIJSON(w, map[string]any{"status": "queued", "delivery": d})
}
