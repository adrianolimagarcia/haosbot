...
function toggle(){if(!pane)ensure();pane.classList.toggle('hidden');document.documentElement.classList.toggle('workbench-open',!pane.classList.contains('hidden'));if(!pane.classList.contains('hidden'))render(current)}
async function get(path){const r=await fetch(path,{headers:auth()});if(!r.ok)throw new Error(await r.text());return r.json()}
async function render(tab){current=tab;body.replaceChildren(e('div','session-loading','Carregando…'));pane.querySelectorAll('.haos-workbench-tabs button').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));try{if(tab==='context')await renderContext();else if(tab==='activity')renderActivity();else if(tab==='automations')await renderJobs();else if(tab==='triggers')await renderTriggers();else renderFiles()}catch(err){body.replaceChildren(e('div','session-loading',err.message))}}
...
function renderFiles(){body.replaceChildren();const row=e('div','control-toolbar'),input=e('input','control-input');input.placeholder='Caminho relativo';const open=e('button','control-button primary','Abrir'),pre=e('pre','control-pre','');open.addEventListener('click',async()=>{const r=await fetch('/api/webui/file-preview?path='+encodeURIComponent(input.value.trim()),{headers:auth()});pre.textContent=r.ok?(await r.json()).content:await r.text()});row.append(input,open);body.append(row,pre)}
