...
func TestServiceRunNowRecordsResult(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		if ok && !got.State.Pending && len(got.State.RunHistory) == 1 {
			...
			record, err := s.ReadRunRecord(got.State.RunHistory[0].RunID); if err != nil { t.Fatal(err) }
...
func TestCorruptStoreIsPreserved(t *testing.T) {
	...
	s := NewService(path, nil); if err := s.Load(); err == nil { t.Fatal("expected corrupt store error") }
...
func TestRunErrorIsRecorded(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }; if err := s.RunNow(job.ID, true); err != nil { t.Fatal(err) }
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		...
		if len(got.State.RunHistory) == 1 {
			...
			raw, err := os.ReadFile(path); if err != nil { t.Fatal(err) }; var stored Store
...
func TestMutationRollbackWhenStoreWriteFails(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	s.storePath = t.TempDir(); renamed := "mutated"
...
func TestDirtyExecutionBlocksAnotherRunUntilPersisted(t *testing.T) {
	root := t.TempDir(); path := filepath.Join(root, "cron", "jobs.json"); var calls atomic.Int32
	...
	if err != nil { t.Fatal(err) }
	s.storePath = t.TempDir(); if err := s.RunNow(job.ID, true); err != nil { t.Fatal(err) }
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		s.mu.Lock(); dirty := s.dirty; active := s.active[job.ID] != nil; s.mu.Unlock()
