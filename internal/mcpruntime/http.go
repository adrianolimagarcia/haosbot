...
func(m *Manager)Close()error{m.mu.Lock();m.clients=nil;m.mu.Unlock();return nil}
...
func(m *Manager)LoadAndRegister(ctx context.Context,reg *tools.Registry,servers map[string]config.MCPServerConfig)error{
 ...
 for _,name:=range names{name:=name;cfg:=servers[name];wg.Add(1);go func(){defer wg.Done();if strings.TrimSpace(cfg.URL)==""{if strings.TrimSpace(cfg.Command)!=""{ch<-loadResult{name:name,err:errors.New("stdio MCP transport is not enabled in this HTTP runtime")};return};ch<-loadResult{name:name,err:errors.New("missing MCP URL")};return};client:=newHTTPClient(name,cfg,m.opt);defs,err:=client.initializeAndList(ctx);ch<-loadResult{name:name,client:client,defs:defs,err:err}}()}
...
type httpClient struct{name string;cfg config.MCPServerConfig;opt Options;mu sync.Mutex;next int64;session string}
func newHTTPClient(name string,cfg config.MCPServerConfig,opt Options)*httpClient{return &httpClient{name:name,cfg:cfg,opt:opt}}
func(c *httpClient)initializeAndList(ctx context.Context)([]toolDef,error){run,cancel:=context.WithTimeout(ctx,12*time.Second);defer cancel();if _,err:=c.request(run,"initialize",map[string]any{"protocolVersion":"2025-06-18","capabilities":map[string]any{},"clientInfo":map[string]any{"name":"haosbot","version":"1"}});err!=nil{return nil,err};_ = c.notify(run,"notifications/initialized",map[string]any{});raw,err:=c.request(run,"tools/list",map[string]any{});if err!=nil{return nil,err};var list struct{Tools []toolDef `json:"tools"`};if err:=json.Unmarshal(raw,&list);err!=nil{return nil,err};return list.Tools,nil}
